...
import { Award, TrendingUp, Calendar, ShoppingBag } from 'lucide-react';
import { useBreakfastReservations, useBreakfastPurchases } from '../lib/queries';

export default function MyBreakfastReservations() {
  const { user } = useAuth();
  const { data: reservations = [], isLoading: resLoading } = useBreakfastReservations(user?.id || '');
  const { data: purchases = [], isLoading: purLoading } = useBreakfastPurchases(user?.id || '');
  const [insight, setInsight] = useState('');
  const [insightLoading, setInsightLoading] = useState(false);

  const loading = resLoading || purLoading;

  React.useEffect(() => {
    if (reservations.length > 0 && !insight) {
      setInsightLoading(true);
      getBreakfastInsights(reservations).then(setInsight).finally(() => setInsightLoading(false));
    }
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-10">
      ...
      {!user && (
        <LoginPrompt 
          ...
          description="Login to track your canteen points, purchase history, and manage your breakfast reservations."
